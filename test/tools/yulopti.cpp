...
public:
	...
	void runSteps(std::string _source, std::string _steps)
	{
		...
		std::cout << AsmPrinter{m_dialect, m_dispenser->generateNewLabels(*m_astRoot, m_dialect)}(*m_astRoot) << std::endl;
	}
	...
	void runInteractive(std::string _source, bool _disambiguated = false)
	{
		...
		while (true)
		{
			...
			try
			{
				...
				_source = AsmPrinter{m_dialect, m_dispenser->generateNewLabels(*m_astRoot, m_dialect)}(*m_astRoot);
			}
			catch (...)
			{
